...
function getLastMessageSender(params)
  local msgs = chats[params.type][params.tab]
  if #msgs > 0 then
    local message = msgs[#msgs]
    local _, _, username = message:find("[^|]*|c0......([^|]*)")
    return username
  end
end

function lastMessageSenderNotMe(params)
  if params.tab == "" then
    return false -- dont care about Lobby messages
  end
  local username = getLastMessageSender(params)
  if username == nil then
    return false -- oopsie whoopsie, we did a fucky wucky
  end
  if username == "System:" then
    return false -- dont care about system messages
  end
...
chat[#chat + 1] = Def.Sound {
  InitCommand = function(self)
    self:load(THEME:GetPathS("ScreenChatOverlay", "notification"))
  end,
  ChatSoundMessageCommand = function(self, params)
    if lastMessageSenderNotMe(params) then
      self:play()
    end
  end
}
...
local chatWindow =
  ...
  ChatMessageCommand = function(self, params)
    ...
    MESSAGEMAN:Broadcast("ChatSound", params)
  end
}
local chatbg
